"""
N8 gRPC Control Center - 安全的gRPC Server
使用合并后的数据库模型
"""
...
from models_merged import Base, Device, Command, AuditLog
...
class ControlCenterConfig:
    ...
    def __init__(self):
        import os
        self.database_url = os.getenv("DATABASE_URL", "postgresql://n8_user:n8_password_2024@postgres:5432/n8_control")
        self.psk = os.getenv("AGENT_PSK", "n8-super-secret-key-2024")
        self.grpc_port = int(os.getenv("GRPC_PORT", "18033"))
        self.heartbeat_interval = int(os.getenv("HEARTBEAT_INTERVAL", "30"))
        self.device_timeout = int(os.getenv("DEVICE_TIMEOUT", "120"))
...
class ControlCenterService(pb2_grpc.ControlCenterServicer):
    ...
    def Heartbeat(self, request: pb2.HeartbeatRequest, context) -> pb2.HeartbeatResponse:
        ...
        try:
            if not self._verify_token(request.device_id, request.token, session):
                return pb2.HeartbeatResponse(success=False, message="Invalid token")
            ...
            if device:
                ...
                return pb2.HeartbeatResponse(success=True, message="OK")
            else:
                return pb2.HeartbeatResponse(success=False, message="Device not found")
        
        except Exception as e:
            self.logger.error(f"Heartbeat error: {e}")
            return pb2.HeartbeatResponse(success=False, message=f"Internal error: {str(e)}")
    ...
    def ReportResult(self, request: pb2.CommandResult, context) -> pb2.ReportResponse:
        ...
        try:
            if not self._verify_token(request.device_id, request.token, session):
                return pb2.ReportResponse(success=False, message="Invalid token")
            ...
            if command:
                ...
                return pb2.ReportResponse(success=True, message="Result recorded")
            else:
                return pb2.ReportResponse(success=False, message="Command not found")
        
        except Exception as e:
            self.logger.error(f"Report result error: {e}")
            return pb2.ReportResponse(success=False, message=f"Internal error: {str(e)}")
    ...
    def StreamLogs(self, request_iterator, context) -> pb2.StreamLogsResponse:
        """流式接收日志"""
        return pb2.StreamLogsResponse(success=True, message="Not implemented yet", bytes_received=0)
